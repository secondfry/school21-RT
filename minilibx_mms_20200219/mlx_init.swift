...
typealias PointerToVoid = @convention(c) (UnsafeRawPointer) -> Void
...
public class MlxMain {
  ...
  public init(_ flag: Int = 0) {
    ...
    if flag == 1 {
      ...
      // for non clickable win, no top menu
    } else {
      NSApp.setActivationPolicy(NSApplication.ActivationPolicy.regular)
    }

    device = MTLCreateSystemDefaultDevice()!
    loopParam = UnsafeMutableRawPointer(&inLoop)
    // dummy addr init

    // Add observer anyway to flush pixels every loop. If loop_hook exists, call it.
    var ocontext = CFRunLoopObserverContext(
      version: 0, info: _mlx_bridge(obj: self), retain: nil, release: nil, copyDescription: nil)
    let observer = CFRunLoopObserverCreate(
      kCFAllocatorDefault, CFRunLoopActivity.beforeWaiting.rawValue, true, 0, createOCallback(),
      &ocontext)
    CFRunLoopAddObserver(CFRunLoopGetMain(), observer, CFRunLoopMode.commonModes)

  }

  public func addWinToList(_ win: MlxWin) { winList.append(win) }
  public func addImgToList(_ img: MlxImg) { imgList.append(img) }

  func doCallLoopHook() {
    if loopHook != nil {
      _ = (unsafeBitCast(loopHook!, to: PointerToVoid.self))(loopParam)
    }
  }
  ...
  public func addLoopHook(_ f: UnsafeMutableRawPointer?, _ p: UnsafeMutableRawPointer) {
    var tcontext = CFRunLoopTimerContext(
      version: 0, info: _mlx_bridge(obj: self), retain: nil, release: nil, copyDescription: nil)
    if loopHook != nil {
    ...
    loopParam = p
    if loopHook != nil {
      loopHookTimer = CFRunLoopTimerCreate(
        kCFAllocatorDefault, 0.0, 0.0001, 0, 0, createTCallback(), &tcontext)
